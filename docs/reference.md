...
**Azure DevOps verification** (Step 1c) runs as a dispatched subagent via Claude Code's `Task` tool when the git remote resolves to `dev.azure.com` or `*.visualstudio.com`. The subagent reads `skills/init-azure-devops/SKILL.md` for its dispatch contract and error-interpretation table, then invokes `scripts/pipeline-init-azure-devops.js` (three subcommands: `verify`, `detect-process-template`, `set-defaults`) to run mechanical `az` CLI calls with structured JSON output. Separation of concerns: LLM-cognitive error interpretation (TF400813 PAT-scope, TF400818 project-not-found, TF401027 unauthorized, extension-missing, network timeout) lives in the SKILL; mechanical invocation lives in the script. Windows-safe via the shared `runWinBin` helper in `scripts/lib/shared.js` (covers `pnpm`, `az`, and any future `.cmd`/`.bat`-on-Windows invocation).

**Windows `.cmd`/`.bat` invocation** — all init helpers that shell out to CLIs distributed as `.cmd` shims on Windows (pnpm via npm-install, az via the Azure CLI MSI) go through `scripts/lib/shared.js`'s `runWinBin(candidates, args, opts)` helper. It handles both PATHEXT resolution (iterates `<bin>.cmd` → `<bin>.exe` → `<bin>` on ENOENT) and Node's CVE-2024-27980 / "BatBadBut" hardening (Node 22+ refuses to invoke `.cmd`/`.bat` via `execFileSync` with `shell: false`; `runWinBin` reroutes to `cmd.exe /d /s /c <bin> <quoted-args...>` and treats cmd.exe's "is not recognized" stderr as an ENOENT-equivalent signal so candidate iteration continues correctly).
